...
# ======= 1. Data Loading =======
sheet_url = "https://docs.google.com/spreadsheets/d/1C8UzXEeRYIuw4mMoYEB1Di3WdRyIhRsOmL0LVvPGNiQ/export?format=csv&gid=239419531"
...
df = load_data()
if df.empty:
    st.error("⚠️ スプレッドシートからデータが取得できませんでした。")
    st.info("💡 **解決方法:** \n\nGoogleスプレッドシート上で「フィルタ」が適用されてデータが隠れていると、ダッシュボードからデータを読み取れない場合があります。\n\n スプレッドシートを開き、上部の **「データ」メニュー ＞ 「フィルタをオフにする」** （または各列の漏斗マークの絞り込みを全解除）をクリックしてから、このページをリロード（再読込）してみてください。")
    st.stop()
