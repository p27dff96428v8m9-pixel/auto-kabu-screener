import streamlit as st
import yfinance as yf
import pandas as pd
import numpy as np
import plotly.graph_objects as go
from datetime import timedelta
import time
import requests
import json
import os

st.set_page_config(page_title="FX 勝率特化ダチE��ュボ�EチE, layout="wide", page_icon="📈")

def calculate_indicators(df):
    if len(df) < 30:
        df['RSI'] = 50
        df['SMA20'] = df['Close']
        df['UpperBB'] = df['Close'] * 1.01
        df['LowerBB'] = df['Close'] * 0.99
        df['ATR'] = df['Close'] * 0.005
        return df
        
    # RSI (14)
    delta = df['Close'].diff()
    gain = delta.clip(lower=0).rolling(14).mean()
    loss = -delta.clip(upper=0).rolling(14).mean()
    rs = gain / loss
    df['RSI'] = 100 - (100 / (1 + rs))
    
    # Bollinger Bands (20, 2.5ρE
    df['SMA20'] = df['Close'].rolling(20).mean()
    df['STD20'] = df['Close'].rolling(20).std()
    df['UpperBB'] = df['SMA20'] + (df['STD20'] * 2.5)
    df['LowerBB'] = df['SMA20'] - (df['STD20'] * 2.5)
    
    # ATR (14)
    df['ATR'] = (df['High'] - df['Low']).rolling(14).mean().bfill()
    
    return df

# ==========================================
# AI決定モーチE(Gemini 2.5 Flash 刁E��)
# ==========================================
@st.cache_data(ttl=3600) # 1時間はキャチE��ュを保持
def get_ai_prediction_cached(data_summary, ticker_name, api_key):
    # 持E��されたGemini 2.5 FlashのURL
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={api_key}"
    
    prompt = f"""
    あなた�E世界最高峰のFXプロトレーダー兼戦略アナリストです、E
    4時間足のチャートデータを�E析し、�Eロの視点から「安定した中長期的な判断」を行ってください、E
    
    【�E析�Eルール、E
    1. **安定性重要E*: 1、E本のローソク足の動きで判断をコロコロ変えなぁE��ください、E
    2. **トレンド把握**: 直迁E00本のチE�Eタから、現在の大きな「流れ�E�上�E・下落・レンジ�E�」を特定してください、E
    3. **エントリー基溁E*: 明確なトレンド�E方向、また�E強力なレジサポ転換を老E�Eし、�Eロとして忁E��「buy�E�買ぁE��」また�E「sell�E�売り）」�EぁE��れかを選択してください、E
    4. **絶対ルール**: 「様子見！Eeutral�E�」�E禁止です。データから最も可能性が高い方向を忁E��断定してください、E
    5. **解説**: 日本語で、なぜその判断に至ったか論理皁E��詳しく説明してください、E
    
    通貨ペア: {ticker_name}
    現在の足: 4時間足
    直迁E00件のチE�Eタ:
    {data_summary}
    
    【�E力形式、E
    忁E��以下�EJSON形式でのみ回答してください、E
    {{
      "decision": "buy" | "sell",
      "reason": "詳細な解説"
    }}
    """
    
    payload = {
        "contents": [{
            "parts": [{"text": prompt}]
        }],
        "generationConfig": {
            "response_mime_type": "application/json",
        }
    }
    
    try:
        response = requests.post(url, json=payload, headers={'Content-Type': 'application/json'})
        response.raise_for_status()
        res_data = response.json()
        text_content = res_data['candidates'][0]['content']['parts'][0]['text']
        res_json = json.loads(text_content)
        return res_json.get("decision", "buy"), res_json.get("reason", "刁E��失敁E)
    except Exception as e:
        return "buy", f"AI刁E��エラー: {e} (注: モチE��吁E'gemini-2.5-flash' が有効か確認してください)"

def get_ai_prediction(df, ticker_name, api_key):
    if not api_key:
        return "neutral", "APIキーが�E力されてぁE��せん、E
    
    # チE�Eタが変わったときだけ�E送するよぁE��、直近�EチE�Eタを文字�E化してハッシュキーにする
    recent_df = df.tail(100).copy()
    data_summary = recent_df[['Open', 'High', 'Low', 'Close', 'RSI']].to_string()
    
    return get_ai_prediction_cached(data_summary, ticker_name, api_key)

# ==========================================
# 全通貨ペア一括シグナル判宁E(高速版)
# ==========================================
@st.cache_data(ttl=300)
def check_all_pair_signals(pairs_dict):
    # 全ペアリストを表示するのみ�E�シグナル判定�E削除�E�E
    return {name: "neutral" for name in pairs_dict.keys()}

# calculate_win_rate functions have been removed


# ==========================================
# サイドバー�E�通貨ペア選択！E
# ==========================================
CURRENCY_PAIRS = {
    "USD/JPY (ドル冁E": "JPY=X", 
    "EUR/USD (ユーロドル)": "EURUSD=X", 
    "GBP/USD (ポンドドル)": "GBPUSD=X",
    "AUD/JPY (豪ドル冁E": "AUDJPY=X", 
    "EUR/JPY (ユーロ冁E": "EURJPY=X", 
    "GBP/JPY (ポンド�E)": "GBPJPY=X",
    "AUD/USD (豪ドル米ドル)": "AUDUSD=X", 
    "USD/CAD (ドルカナダ)": "CAD=X", 
    "USD/CHF (ドルスイス)": "CHF=X",
    "NZD/USD (NZドル米ドル)": "NZDUSD=X", 
    "AUD/CAD (豪ドルカナダ)": "AUDCAD=X", 
    "AUD/NZD (豪ドルNZドル)": "AUDNZD=X",
    "CAD/JPY (カナダ冁E": "CADJPY=X", 
    "CAD/CHF (カナダスイス)": "CADCHF=X", 
    "EUR/CAD (ユーロカナダ)": "EURCAD=X",
    "EUR/NZD (ユーロNZドル)": "EURNZD=X", 
    "GBP/NZD (ポンドNZドル)": "GBPNZD=X", 
    "NZD/CAD (NZドルカナダ)": "NZDCAD=X",
    "NZD/CHF (NZドルスイス)": "NZDCHF=X", 
    "NZD/JPY (NZドル冁E": "NZDJPY=X", 
    "CHF/JPY (スイス冁E": "CHFJPY=X",
    "AUD/CHF (豪ドルスイス)": "AUDCHF=X", 
    "EUR/AUD (ユーロ豪ドル)": "EURAUD=X", 
    "EUR/CHF (ユーロスイス)": "EURCHF=X",
    "EUR/GBP (ユーロポンチE": "EURGBP=X", 
    "GBP/AUD (ポンド豪ドル)": "GBPAUD=X", 
    "GBP/CHF (ポンドスイス)": "GBPCHF=X",
    "GBP/CAD (ポンドカナダ)": "GBPCAD=X",
    "GOLD (釁E": "GC=F", 
    "Bitcoin (BTC)": "BTC-USD"
}

st.sidebar.image("https://img.icons8.com/color/96/000000/line-chart.png", width=60)
st.sidebar.title("FX自動監視シスチE��")
st.sidebar.markdown("🤁E**AI刁E��完�E特化モーチE* 稼働中")

# CSS インジェクション (スタイル調整のみ、アニメーションは廁E��)
st.markdown("""
<style>
    .stRadio > label { display: none; }
</style>
""", unsafe_allow_html=True)

st.sidebar.markdown("---")

# シグナル一括チェチE��を実衁E
all_signals = check_all_pair_signals(CURRENCY_PAIRS)
st.sidebar.markdown("---")

# ラジオボタンの選択肢�E�安定�Eため、名前�Eみをリストにする�E�E
radio_options = list(CURRENCY_PAIRS.keys())

selected_idx = 0
current_p = st.session_state.get('selected_pair', radio_options[0])
if current_p in radio_options:
    selected_idx = radio_options.index(current_p)

# AIの状態をリセチE��するコールバック関数
def reset_ai_state():
    st.session_state.ai_decision = "neutral"
    st.session_state.ai_reason = "通貨ペアが変更されました、EI刁E��を�E動的に開始しまぁE.."

def format_pair_label(name):
    return f"🌐 {name}"

selected_name = st.sidebar.radio(
    "**刁E��するペアを選抁E*",
    radio_options,
    index=selected_idx,
    on_change=reset_ai_state,
    key="currency_selector_fixed",
    format_func=format_pair_label
)

# 通貨ペア惁E��の確宁E
st.session_state.selected_pair = selected_name
ticker = CURRENCY_PAIRS[selected_name]
current_signal = all_signals.get(selected_name, "neutral")

# 選択中のペアの状態を強調表示
if current_signal == "buy":
    st.sidebar.success(f"⬁E��E{selected_name}: 買ぁE��グナル発生中")
elif current_signal == "sell":
    st.sidebar.error(f"⬁E��E{selected_name}: 売りシグナル発生中")

st.sidebar.subheader("🤁EAI決定モーチE)
# AI刁E��を常にオンにする�E�無効化不可�E�E
use_ai = True 
st.sidebar.info("AI刁E��は常時有効化されてぁE��ぁE)
gemini_key = "AIzaSyAwXJBVQ2GmpgLfNCzvz3h-VfEZ3HykoGA"  # 提供されたAPIキー
if use_ai:
    # キー入力を非表示にするか、確認用に残す (提供されたキーを優允E
    input_key = st.sidebar.text_input("Gemini API Key", type="password", value=gemini_key)
    if input_key:
        gemini_key = input_key
    if not gemini_key:
        st.sidebar.warning("刁E��にはAPIキーが忁E��です、E)

# 期間と足の設定！E時間足で固定！E
period = "60d" 
interval = "4h"

# ==========================================
# メイン画面
# ==========================================
st.title(f"🤁E{selected_name} - AI完�E特匁E刁E��ダチE��ュボ�EチE)
st.markdown("従来の鉁E��ロジチE��をAIが吸収�E進化、Eemini 2.0 Flashが、チャート�E全チE�Eタを多角的に刁E��し、次の一手を断定します、E)

# チE�Eタの取得（キャチE��ュを活用してAPI負荷を軽減！E
with st.spinner(f"{selected_name}のチE�Eタを取得中..."):
    t = yf.Ticker(ticker)
    df = t.history(period=period, interval=interval)
    
if df.empty:
    st.error("チE�Eタを取得できませんでした。休場時間、あるいはチE��チE��ーが無効の可能性があります、E)
else:
    # 持E���E計箁E
    df = calculate_indicators(df)
    current_price = float(df['Close'].iloc[-1])
    
    # 勝率の計算ロジチE��は削除
    outcomes_record = []
    
    # ==========================================
    # プロ仕様�Eチャート描画 (Plotly)
    # ==========================================
    fig = go.Figure()
    
    # ローソク足
    fig.add_trace(go.Candlestick(x=df.index,
                    open=df['Open'], high=df['High'],
                    low=df['Low'], close=df['Close'],
                    name="価格",
                    increasing_line_color='blue',   
                    decreasing_line_color='red',    
                    increasing_fillcolor='blue',
                    decreasing_fillcolor='red'
                    ))
    
    # ボリンジャーバンド描画
    fig.add_trace(go.Scatter(x=df.index, y=df['UpperBB'], mode='lines', line=dict(color='rgba(255, 255, 0, 0.5)', width=1, dash='dash'), name='Upper BB (2.5ρE'))
    fig.add_trace(go.Scatter(x=df.index, y=df['LowerBB'], mode='lines', line=dict(color='rgba(0, 255, 255, 0.5)', width=1, dash='dash'), name='Lower BB (2.5ρE', fill='tonexty', fillcolor='rgba(255,255,255,0.05)'))
    fig.add_trace(go.Scatter(x=df.index, y=df['SMA20'], mode='lines', line=dict(color='rgba(255, 255, 255, 0.3)', width=1), name='SMA 20'))

    # 鉁E��サインのプロチE��は削除
                                 
    # AI判断のプロチE��
    ai_decision = "neutral"
    ai_reason = ""
    if use_ai and gemini_key:
        # 自動�E析また�Eボタンによる手動更新
        if st.session_state.get('ai_decision', 'neutral') == 'neutral' or st.sidebar.button("🤁EAI判定を更新"):
            # キャチE��ュをクリアして強制更新したぁE��合�Eサイドバーボタンで対応可能に
            if st.session_state.get('ai_decision', 'neutral') != 'neutral':
                st.cache_data.clear()
            
            with st.spinner("Gemini 2.5 Flash が相場を�E析中..."):
                ai_decision, ai_reason = get_ai_prediction(df, selected_name, gemini_key)
                st.session_state.ai_decision = ai_decision
                st.session_state.ai_reason = ai_reason
        
        ai_decision = st.session_state.get('ai_decision', 'neutral')
        ai_reason = st.session_state.get('ai_reason', 'サイドバーのボタンを押して刁E��を開始してください、E)
        
        if ai_decision != "neutral":
            color = "#00ff00" if ai_decision == "buy" else "#ff3333"
            symbol = "star"
            fig.add_trace(go.Scatter(x=[df.index[-1]], y=[current_price], mode='markers+text',
                                     marker=dict(symbol=symbol, size=25, color=color, line=dict(color="white", width=2)),
                                     text=[f"AI: {ai_decision.upper()}"] , textposition="top center",
                                     textfont=dict(color=color, size=16, weight='bold'),
                                     name='AI判宁E))
                                 
    # 勝敗のプロチE��
    win_x = [rt[1] for rt in outcomes_record if 'win' in rt[0]]
    win_y = [rt[2] for rt in outcomes_record if 'win' in rt[0]]
    if win_x:
        fig.add_trace(go.Scatter(x=win_x, y=win_y, mode='text', text=["⭁E] * len(win_x), textposition="middle center", textfont=dict(size=14), name='利確'))

    loss_x = [rt[1] for rt in outcomes_record if 'loss' in rt[0]]
    loss_y = [rt[2] for rt in outcomes_record if 'loss' in rt[0]]
    if loss_x:
        fig.add_trace(go.Scatter(x=loss_x, y=loss_y, mode='text', text=["❁E] * len(loss_x), textposition="middle center", textfont=dict(size=14), name='損�E'))

    # 現在状態�E判定と背景色
    current_atr = df['ATR'].iloc[-1]
    current_rsi = df['RSI'].iloc[-1]
    sma20 = df['SMA20'].iloc[-1]
    upper_bb = df['UpperBB'].iloc[-1]
    lower_bb = df['LowerBB'].iloc[-1]
    
    # AI判断に基づぁE��背景色とチE��ストを設宁E
    ai_decision = st.session_state.get('ai_decision', 'neutral')
    
    if ai_decision == "buy":
        bg_color = "#1a4d29"
        current_trend_text = "🚀 AI判宁E 買ぁE(BUY) 優勢"
        current_trend_color = "#00ff00"
    elif ai_decision == "sell":
        bg_color = "#4d1a1a"
        current_trend_text = "💥 AI判宁E 売めE(SELL) 優勢"
        current_trend_color = "#ff0000"
    else:
        bg_color = "#111111"
        current_trend_text = "⌁EAI刁E��中..."
        current_trend_color = "#ffffff"

    info_text = (
        f"<b>【AI刁E��スチE�Eタス、E/b><br>"
        f"<span style='color:{current_trend_color}; font-size:18px;'><b>{current_trend_text}</b></span><br>"
        f"現在値: {current_price:.4f}<br>"
        f"RSI(14): <b>{current_rsi:.1f}</b><br><br>"
        f"────────────────<br>"
        f"🤁E<b>AI完�E特化モード稼働中</b>"
    )
    
    fig.update_layout(
        title=f"【{selected_name}、E4時間足 - AI刁E��チャーチE,
        yaxis_title="価格 (Price)",
        xaxis_title="日晁E(4H)",
        template="plotly_dark",
        plot_bgcolor=bg_color,
        paper_bgcolor="#111111",
        height=700,
        margin=dict(l=0, r=0, b=0, t=40),
        legend=dict(yanchor="top", y=0.99, xanchor="left", x=0.01),
        dragmode='pan'
    )
    
    fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(255, 255, 255, 0.1)', rangeslider_visible=False)
    
    default_start_idx = max(0, len(df) - 60)
    y_min_val = df['Low'].iloc[default_start_idx:].min() * 0.995
    y_max_val = df['High'].iloc[default_start_idx:].max() * 1.005
    fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(255, 255, 255, 0.1)', fixedrange=False, autorange=False, range=[y_min_val, y_max_val])
    
    fig.add_annotation(
        x=0.99, y=0.98, xref="paper", yref="paper",
        text=info_text,
        showarrow=False,
        font=dict(size=14, color="white"),
        align="left",
        bgcolor="rgba(0,0,0,0.7)",
        bordercolor="gray", borderwidth=1, borderpad=8
    )
    
    st.plotly_chart(fig, use_container_width=True, config={'scrollZoom': True}, key=f"chart_{selected_name}")

    if use_ai:
        st.markdown("---")
        st.subheader("🤁EGemini 2.5 Flash AI刁E��レポ�EチE)
        if ai_decision == "buy":
            st.success(f"🚀 **AI判宁E 買ぁE(BUY)**\n\n{ai_reason}")
        else:
            st.error(f"💥 **AI判宁E 売めE(SELL)**\n\n{ai_reason}")

    # ==========================================
    # 刁E��パネル
    # ==========================================
    col1, col2 = st.columns([1, 1.5])
    
    with col1:
        st.subheader("💡 現在のパラメーター値")
        st.write(f"- **現在価格**: `{current_price:.4f}`")
        st.write(f"- **RSI (14)**: `{current_rsi:.1f}`")
        st.write(f"- **20 SMA (中央緁E**: `{sma20:.4f}`")
        st.write(f"- **上部バンチE(+2.5ρE**: `{upper_bb:.4f}`")
        st.write(f"- **下部バンチE(-2.5ρE**: `{lower_bb:.4f}`")

    with col2:
        st.subheader("🤁EAI完�E監視�E刁E��中")
        st.info("AIがチャート�E形状、テクニカル持E��、�EラチE��リチE��を総合皁E��判断してぁE��す。結果は上�Eレポ�Eトを確認してください、E)
